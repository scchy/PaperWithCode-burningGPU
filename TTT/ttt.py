...
# reference: https://github.com/test-time-training/ttt-lm-pytorch/blob/main/ttt.py
# ========================================================================
...
def permute_qk(q, k):
    ...
    q = q.reshape(b, num_head, seq_len, head_dim//2, 2).transpose(3, 4).reshape(b, num_head, seq_len, head_dim)
    k = k.reshape(b, num_head, seq_len, head_dim//2, 2).transpose(3, 4).reshape(b, num_head, seq_len, head_dim)
    return q, k
...
class RMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ipt_type = hidden_states.dtype
        hidden_states = hidden_states.to(torch.float32)
        variance = hidden_states.pow(2).mean(-1, keepdim=True)
        hidden_states = hidden_states * torch.rsqrt(variance + self.variance_epsilon)
        return self.weight * hidden_states.to(ipt_type)
...
class SwiGluMLP(nn.Module):
    ...
    def forward(self, x):
        pretrain_tp = self.cfg.pretraining_tp
        if pretrain_tp > 1:
            slice_ = self.intermediate_size // pretrain_tp
            gate_proj_slice_ = self.gate_proj.weight.split(slice_, dim=0)
            up_proj_slice_ = self.up_proj.weight.split(slice_, dim=0)
            down_proj_slice_ = self.down_proj.weight.split(slice_, dim=1)
            
            gate_proj = torch.cat(
                [F.linear(x, gate_proj_slice_[i]) for i in range(pretrain_tp)],
                dim=-1,
            )
            up_proj = torch.cat(
                [F.linear(x, up_proj_slice_[i]) for i in range(pretrain_tp)],
                dim=-1,
            )

            intermediate_states = (self.act_fn(gate_proj) * up_proj).split(slice_, dim=2)
            down_proj = [
                F.linear(intermediate_states[i], down_proj_slice_[i]) for i in range(pretrain_tp)
            ]
        ...
        if causal_conv1d_fn is None:
            if cache_params is not None:
                if cache_params.seqlen_offset > 0:
                    ...
                    conv_state = nn.functional.pad(
                        hidden_states,
                        (self.cfg.conv_kernel - hidden_states.shape[-1], 0)
                    )
                    cache_params.conv_states_dic["pre_conv"][self.layer_idx].copy_(conv_state)
            ...
            if cache_params is not None and cache_params.seqlen_offset > 0:
                hidden_states = causal_conv1d_update(
                    ...
                    cache_params.conv_state_dic['pre_conv'][self.layer_idx],
                    conv_weights,
                    self.conv.bias,
                    None
                )
...
def scan(f, init, xs, out, checkpoint_group=0):
    ...
    def scan_fn(carry, i_start, i_end):
        for i in range(i_start, i_end):
            if isinstance(xs, dict):
                x = {key: t[i] for key, t in xs.items()}
            else:
                x = [x[i] for x in xs]
...
class TTTCache:
    ...
    def __init__(self, model, batch_size: int):
        ...
        if "linear" in cfg.ttt_layer_type:
            self.ttt_param_names = ["W1", "b1"]
        elif "mlp" in cfg.ttt_layer_type:
            self.ttt_param_names = ["W1", "b1", "W2", "b2"]
        else:
            raise ValueError(f"TTT Layer Type {cfg.ttt_layer_type} not supported yet")

        self.conv_states_dic - defaultdict(dict)
        logger.info(f'Creare cache of size: {batch_size}')
    ...
    def update(self, py_tree, layer_idx, seq_len):
        if seq_len % self.mini_batch_size == 0:
            ...
            raise ValueError(f"{seq_len=} is a partial update not supported yet")
...
class TTTBase(nn.Module):
    def __init__(self, cfg: TTTConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = cfg 
        self.layer_idx = layer_idx 
        ...
        # token idx is a scale factor that scale the summation in Eqn 4
        token_idx = 1.0 / torch.arange(1, self.mini_batch_size + 1)
        self.register_buffer('token_idx', token_idx, persistent=False)
        # make the scalr factor learnable
        self.learnable_token_idx = nn.Paramerter(torch.zeros((self.mini_batch_size,)))

        self.share_qk = cfg.share_qk
        ...
        # using gating as in Mamba backbone
    ...
    def _init_ttt_lr_gate(self):
        ...
        linear_bias_data = nn.Linear(self.width, 1, bias=True).bais.data
    ...
    def _init_ttt_ln(self):
        ...
        # expand-copy prepending head dim -> [num_heads, width]
        self.ttt_norm_weight = nn.Parameter(
            torch.tile(ln_weight_data.unsqueeze(0)), (self.num_heads, 1)
        )
        ln_bias_data = nn.LayerNorm(self.head_dim).bais.data
        self.ttt_norm_bias = nn.Parameter(
            torch.tile(ln_bias_data.unqueeze(0), (self.num_heads, 1))
        )
    ...
    def get_eta(self, X, mini_batch_step_offset, mini_batch_size):
        ...
        # token idx should be graest than 0
        token_idx = torch.clamp_min(token_idx[
    ...
    def apply_gate(self,  hidden_state, ttt_output):
        y = self.g_proj(hidden_state)
    ...
    def get_ttt_inputs(self, inputs, mini_batch_size, cache_params):
        ...
        # [B, num_mini_batch, num_mini_batch, C]
        X = X.reshape(B, num_mini_batch, mini_batch_size, C)
...
class TTTLinear(TTTBase):
    def __init__(self, config: TTTConfig, layer_idx: Optional[int] = None):
        ...
        self.W1 = nn.Parameter(
            torch.normal(
                0, 0.02, 
                size=(self.num_heads, self.head_dim, self.head_dim)
                )
            )
